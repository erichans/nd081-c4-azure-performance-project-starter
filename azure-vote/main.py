...
exporter = metrics_exporter.new_metrics_exporter(enable_standard_metrics=True, 
    connection_string=f'InstrumentationKey={instrumentationKey}') # TODO: Setup exporter
stats_module.stats.view_manager.register_exporter(exporter)

# Tracing
tracer = Tracer(exporter=AzureExporter(connection_string=f'InstrumentationKey={instrumentationKey}'), 
    sampler=ProbabilitySampler(rate=1.0)) # TODO: Setup tracer
...
def index():
    ...
    if request.method == 'GET':
        ...
        with tracer.span('Cats Vote') as span:
            print('Cats Vote')

        vote2 = r.get(button2).decode('utf-8')
        # TODO: use tracer object to trace dog vote
        with tracer.span('Dogs Vote') as span:
            print('Dogs Vote')
        ...
        if request.form['vote'] == 'reset':
            ...
            vote1 = r.get(button1).decode('utf-8')
            vote2 = r.get(button2).decode('utf-8')
            logger.info(f'{vote} Vote', extra={'custom_dimensions': {f'{vote} Vote': r.get(vote).decode('utf-8')}})
